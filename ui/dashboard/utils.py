...
# API_URL is read from the environment so no IP/hostname is hardcoded here.
# Set API_URL in your .env file or container environment.
...
def predict_image(image_file):
    ...
    try:
        ...
        r = requests.post(f"{API_URL}/predict", files=files, timeout=REQUEST_TIMEOUT)
        r.raise_for_status()
        return r.json()

    except requests.exceptions.ConnectionError:
        return {"error": "Cannot reach the API server. Check your network or API_URL."}
...
def load_results():
    csv_path = DATA_DIR / "evaluation" / "results.csv"
    return _safe_load_csv(
        csv_path,
        fallback_columns=[
            "model", "accuracy", "f1_macro", "f1_weighted",
            "params", "inference_ms_per_batch", "file_size_mb",
        ],
    )
